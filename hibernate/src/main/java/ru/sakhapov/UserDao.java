...
public class UserDao {
    ...
    public void update(User user) {
        ...
        try (Session session = SessionFactoryMaker.getSessionFactory().openSession()) {
            ...
            session.merge(user);
            tx.commit();
            logger.info("Юзер обновлён: {}", user.getEmail());
        } catch (ConstraintViolationException e) {
            if (tx != null && tx.getStatus() == TransactionStatus.ACTIVE) tx.rollback();
            logger.warn("Обновление: юзер с email {} уже существует", user.getEmail());
        } catch (HibernateException e) {
            if (tx != null && tx.getStatus() == TransactionStatus.ACTIVE) tx.rollback();
            logger.error("Ошибка Hibernate при обновлении: {}", e.getMessage(), e);
        } catch (PersistenceException e) {
            if (tx != null && tx.getStatus() == TransactionStatus.ACTIVE) tx.rollback();
            logger.error("Ошибка БД при обновлении: {}", e.getMessage(), e);
        } catch (Exception e) {
            if (tx != null && tx.getStatus() == TransactionStatus.ACTIVE) tx.rollback();
            logger.error("Неизвестная ошибка при обновлении: {}", e.getMessage(), e);
        }
    }
